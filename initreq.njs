exports.initGET = function(req, pre, cb) { pre._GET = {};
    ...
    cb(); };
exports.initPOST = function(req, pre, cb) { pre._POST = {};
    var body = '';
    ...
    req.on('end', function() {
        ...
        for (var p=0; p < pairs.length; ++p) {
            var pair = pairs[p].split('='); pre._POST[pair[0]] = pair[1];
        }
        cb(); });
};
exports.initCOOKIE = function(req, pre, cb) { pre._COOKIE = {};
    if (req.headers.cookie) {
        var cookies = req.headers.cookie.split(';'); for (var c=0; c < cookies.length; ++c) {
            var pair = cookies[c].split('=');
            pre._COOKIE[pair[0]] = pair[1];
        }
    }
    cb(); };
exports.initREQUEST = function(req, pre, cb) { pre._REQUEST = {};
    if (pre._GET) {
        for (var k in pre._GET) { pre._REQUEST[k] = pre._GET[k];
        } }
    if (pre._POST) {
    ...
    if (pre._COOKIE) {
        for (var k in pre._COOKIE) { pre._REQUEST[k] = pre._COOKIE[k];
        } }
    cb(); };
...
exports.initSESSION = function(req, pre, cb) {
    ...
    if ((typeof sessions[id]) == 'undefined') { sessions[id] = {};
    }
